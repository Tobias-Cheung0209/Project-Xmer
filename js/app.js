...
const App = (function () {
  ...
  function renderTravelOverview(tab) {
    ...
    const maps = `<div class="travel-maps">${imageMap('中国','images/china-map.png?v=54',chinaBox,chinaPins)}${imageMap('世界','images/world-map.png?v=54',worldBox,worldPins)}</div>`;
  ...
  function calendarUid(e){return `xiaoman-${e.sourceCollection||e.category}-${e.sourceId||'auto'}-${e.date}-${String(e.title||'event').replace(/[^\w\u4e00-\u9fff]+/g,'-')}@xmer`;}
  function icsEscape(v){return String(v||'').replace(/\\/g,'\\\\').replace(/\r?\n/g,'\\n').replace(/,/g,'\\,').replace(/;/g,'\\;');}
  function icsFold(line){let rows=[''],bytes=0;for(const ch of String(line)){const n=new TextEncoder().encode(ch).length;if(bytes+n>73){rows.push(' '+ch);bytes=1+n;}else{rows[rows.length-1]+=ch;bytes+=n;}}return rows.join('\r\n');}
  function eventIcs(e){const day=e.date.replace(/-/g,''),next=new Date(e.date+'T00:00:00');next.setDate(next.getDate()+1);const stamp=new Date().toISOString().replace(/[-:]/g,'').replace(/\.\d{3}Z$/,'Z'),lines=['BEGIN:VEVENT',`UID:${calendarUid(e)}`,`DTSTAMP:${stamp}`];if(e.time&&/^\d{1,2}:\d{2}/.test(e.time)){const [h,m]=e.time.match(/\d+/g),end=new Date(`${e.date}T${String(h).padStart(2,'0')}:${m}:00`);end.setHours(end.getHours()+1);const endKey=`${dateKey(end).replace(/-/g,'')}T${String(end.getHours()).padStart(2,'0')}${String(end.getMinutes()).padStart(2,'0')}00`;lines.push(`DTSTART:${day}T${String(h).padStart(2,'0')}${m}00`,`DTEND:${endKey}`);}else lines.push(`DTSTART;VALUE=DATE:${day}`,`DTEND;VALUE=DATE:${dateKey(next).replace(/-/g,'')}`);lines.push(`SUMMARY:${icsEscape(e.title)}`,`DESCRIPTION:${icsEscape(`来自 Xmer · ${(CAL_CATS[e.category]||['事项'])[0]}`)}`,'END:VEVENT');return lines.map(icsFold).join('\r\n');}
  function markCalendarExports(events){events.forEach(e=>{const uid=calendarUid(e),old=Store.getList('calendarExports').find(x=>x.uid===uid);if(old)Store.updateRecord('calendarExports',old._id,{exportedAt:new Date().toISOString()});else Store.addRecord('calendarExports',{uid,sourceCollection:e.sourceCollection||'',sourceId:e.sourceId||'',date:e.date,title:e.title,exportedAt:new Date().toISOString()});});}
  async function exportCalendarEvents(events,fileName){if(!events.length){alert('这个范围内没有可导出的未完成事项');return;}const text=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Xmer//Calendar//ZH','CALSCALE:GREGORIAN','METHOD:PUBLISH',...events.map(eventIcs),'END:VCALENDAR'].join('\r\n'),blob=new Blob([text],{type:'text/calendar;charset=utf-8'}),file=new File([blob],fileName,{type:'text/calendar'});if(navigator.canShare&&navigator.canShare({files:[file]})){try{await navigator.share({title:'添加到系统日历',files:[file]});markCalendarExports(events);renderContent();return;}catch(err){if(err.name==='AbortError')return;}}const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=file.name;a.click();markCalendarExports(events);setTimeout(()=>URL.revokeObjectURL(a.href),1000);renderContent();}
  function openCalendarExport(){openModal('导出到系统日历',`<div class="calendar-export-note">生成标准 .ics 文件后，由 Apple 日历、Google 日历或手机系统日历确认添加。导出不会将任务标记为完成。</div><div class="calendar-export-options"><button data-export-days="7"><b>未来 7 天</b><small>近期安排</small></button><button data-export-days="30"><b>未来 30 天</b><small>适合月度同步</small></button><button data-export-days="3650"><b>全部未完成</b><small>最长未来 10 年</small></button></div>`);document.querySelectorAll('[data-export-days]').forEach(b=>b.onclick=()=>{const days=Number(b.dataset.exportDays),end=new Date(todayKey()+'T00:00:00');end.setDate(end.getDate()+days);const rows=collectTimeEvents(todayKey(),dateKey(end));closeModal();exportCalendarEvents(rows,`xmer-calendar-${todayKey()}-${days===3650?'all':days+'d'}.ics`);});}
  function renderGlobalCalendar() {
    ...
    const dayEvents=(byDate[globalCalendarDate]||[]).map((e,i)=>{const cat=CAL_CATS[e.category]||['事项','•'],exported=Store.getList('calendarExports').some(x=>x.uid===calendarUid(e));return `<div class="global-agenda-row"><i class="cat-${e.category}">${cat[1]}</i><button data-event-open="${esc(e.sourceModule||'')}" data-event-tab="${esc(e.sourceTab||'')}"><b>${esc(e.title)}</b><small>${esc(e.time||cat[0])}${e.predicted?' · 预计':''}</small></button><button class="agenda-calendar ${exported?'exported':''}" data-calendar-event="${i}" aria-label="${exported?'再次导出，可能重复':'添加到系统日历'}">${exported?'✓':'＋'}</button>${e.sourceCollection==='plans'?`<button class="agenda-done" data-event-done="${e.sourceId}">✓</button>`:''}</div>`;}).join('')||'<div class="empty-hint">这一天没有安排</div>';
    const filters=`<button id="calendar-export" class="calendar-export-main">↗ 导出日历</button>`+['all',...Object.keys(CAL_CATS)].map(k=>`<button data-cal-filter="${k}" class="${globalCalendarFilter===k?'active':''}">${k==='all'?'全部':CAL_CATS[k][0]}</button>`).join('');
  ...
  function bindGlobalCalendar(){
    ...
    const dayAll=collectTimeEvents(globalCalendarDate,globalCalendarDate),currentEvents=globalCalendarFilter==='all'?dayAll:dayAll.filter(e=>e.category===globalCalendarFilter);document.querySelectorAll('[data-calendar-event]').forEach(b=>b.onclick=()=>{const e=currentEvents[Number(b.dataset.calendarEvent)];if(!e)return;const already=Store.getList('calendarExports').some(x=>x.uid===calendarUid(e));if(already&&!confirm('这项日程以前导出过，再次添加可能在系统日历中产生重复。仍要继续吗？'))return;exportCalendarEvents([e],`xmer-${e.date}.ics`);});
    const exportBtn=document.getElementById('calendar-export');if(exportBtn)exportBtn.onclick=openCalendarExport;
  ...
  function renderInvestMarket() {
    let cache={}; try{cache=JSON.parse(localStorage.getItem('xmer:marketCache')||'{}');}catch(_){}
  ...
  const cookingLogTab={id:'cookingLogs',name:'炉火小记',collection:'cookingLogs',fields:[{key:'date',label:'日期',type:'date',required:true},{key:'recipeName',label:'关联菜谱',type:'text',required:true},{key:'taste',label:'味道状态',type:'selectOther',options:['偏淡','刚好','偏咸','偏甜','偏辣'],def:'刚好'},{key:'changes',label:'本次调整',type:'textarea'},{key:'nextChange',label:'下次改进',type:'textarea'},{key:'success',label:'是否成功',type:'checkbox',def:true},{key:'worthRepeating',label:'值得常做',type:'checkbox'},{key:'photo',label:'成品照片',type:'image'}]};
  ...
  function renderKitchen(tab){
    ...
    if(tab.id==='gallery')return renderKitchenGallery();
    const nav=kitchenSubnav(kitchenCookingView,[['recipes','菜谱库'],['ideas','灵感备忘'],['logs','炉火小记'],['notes','厨房笔记'],['grocery','采购单']],'kitchen-view');
    return nav+renderKitchenCookingView();
  }
  function renderKitchenGallery(){
    const works=Store.getList('cookingLogs').filter(r=>r.photo).slice().sort((a,b)=>(b.date||'').localeCompare(a.date||''));
    const dishCounts={};Store.getList('cookingLogs').forEach(r=>{const key=(r.recipeName||'未命名菜品').trim();dishCounts[key]=(dishCounts[key]||0)+1;});
    const head=`<section class="kitchen-gallery-head"><div><small>把每一次开火，留成一页食光</small><strong>${works.length} 幅成品 · ${Object.keys(dishCounts).length} 道菜</strong></div><button data-kitchen-add="log">＋ 留下新作</button></section>`;
    if(!works.length)return head+kitchenEmpty('📷','炉火小记中添加成品照片后，会自动陈列在这里');
    return head+`<div class="kitchen-gallery">${works.map(r=>`<article><button data-kitchen-edit="${r._id}" data-kind="log" aria-label="查看并编辑 ${esc(r.recipeName||'成品')}"><img src="${esc(r.photo)}" alt="${esc(r.recipeName||'成品照片')}"><span><b>${esc(r.recipeName||'未命名菜品')}</b><small>${esc(r.date||'日期未记')} · 做过 ${dishCounts[(r.recipeName||'未命名菜品').trim()]||1} 次</small>${r.taste?`<em>${esc(r.taste)}</em>`:''}${r.worthRepeating?'<i>常做</i>':''}</span></button></article>`).join('')}</div>`;
  }
  ...
  function renderKitchenCookingView(){
    ...
    if(kitchenCookingView==='logs'){const rows=Store.getList('cookingLogs').slice().sort((a,b)=>(b.date||'').localeCompare(a.date||''));return `<div class="kitchen-view-head"><div><small>每次开火都留下改进</small><strong>${rows.length} 篇炉火小记</strong></div><button data-kitchen-add="log">＋ 记一次开火</button></div>`+(rows.length?rows.map(r=>`<article class="cooking-log-card">${r.photo?`<img src="${esc(r.photo)}">`:'<i>🍽️</i>'}<div><small>${esc(r.date||'')}${r.taste?' · '+esc(r.taste):''}</small><h3>${esc(r.recipeName)}</h3>${r.changes?`<p>本次：${esc(r.changes)}</p>`:''}${r.nextChange?`<em>下次：${esc(r.nextChange)}</em>`:''}</div><span><button data-kitchen-edit="${r._id}" data-kind="log">编辑</button><button data-kitchen-del="${r._id}" data-col="cookingLogs">删除</button></span></article>`).join(''):kitchenEmpty('🍽️','开火做完一道菜后，记下这次的心得'));}
  ...
  function miniBars(items, max) {
    ...
    return `<div class="mini-bars">` + items.map(i => {
      ...
      return `<div class="mbar"><span>${esc(i.label)}</span><div class="mbar-track"><div class="mbar-fill" style="width:${pct}%"></div></div><b title="${esc(String(v))}">${compactMoney(v)}</b></div>`;
    }).join('') + `</div>`;
  }
  ...
  function openSettings() {
    ...
    const xmMode=Store.getSetting('xmerMode','dynamic');
    openModal('设置与备份', `<div class="form-row"><label>昵称</label><input type="text" id="set-name" value="${esc(name)}"></div>
      ...
      <div class="form-row"><label>Xmer 陪伴模式</label><select id="set-xiaoman-mode"><option value="dynamic" ${xmMode==='dynamic'?'selected':''}>动态陪伴</option><option value="idle" ${xmMode==='idle'?'selected':''}>只待机</option><option value="hidden" ${xmMode==='hidden'?'selected':''}>隐藏 Xmer</option></select><small class="setting-help">动态陪伴只在安全范围内低频活动；输入、滚动和弹窗时自动暂停。</small></div>
    ...
    document.getElementById('settings-confirm').onclick = () => {
      ...
      Store.setSetting('xmerMode',document.getElementById('set-xiaoman-mode').value);if(typeof Xiaoman!=='undefined'&&Xiaoman.applyMode)Xiaoman.applyMode(document.getElementById('set-xiaoman-mode').value);
  ...
  function openQuickAdd() {
    openModal('Xmer · 快捷新建', `<div class="quick-menu"><button data-quick="plans">📝 记待办</button><button data-quick="flows">💶 记一笔收支</button><button data-quick="daily">🏃 记运动打卡</button><button data-quick="skincare">🧴 记护理</button><button data-quick="stockLog">📈 写投资复盘</button></div>`);
  ...
  async function exportBackup() {
    ...
    const file = new File([blob], 'xmer-backup-' + todayKey() + '.json', {type:'application/json'});
    if (navigator.canShare && navigator.canShare({files:[file]})) { try { await navigator.share({title:'Xmer JSON 备份',files:[file]}); return; } catch(e) { if(e.name==='AbortError') return; } }
